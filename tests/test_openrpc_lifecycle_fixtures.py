...
FIXTURE_ROOT = REPO_ROOT / "tests" / "fixtures" / "openrpc" / "lifecycle_states"


class OpenRpcLifecycleFixtureTests(unittest.TestCase):
    def test_checked_in_lifecycle_and_replacement_fixtures_render_expected_output(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            output_dir = Path(temp_dir) / "openrpc-lifecycle"
            ...
            self.assertEqual(
                cli_main(
                    [
                        ...
                        str(output_dir),
                        "--overview-title",
                        "Lifecycle Example OpenRPC",
                    ]
                ),
                0,
            )

            spec_page = (output_dir / "specs" / "payment-api.mdx").read_text(encoding="utf-8")

            self.assertIn("## Version Change Timeline", spec_page)
            self.assertIn("## Table of Contents", spec_page)
            self.assertIn("## Methods", spec_page)
